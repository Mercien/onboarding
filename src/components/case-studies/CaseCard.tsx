...
export function CaseCard({
  ...
  return (
    <Link
      ...
      className="group bg-navy-card border border-white/7 rounded-2xl overflow-hidden transition-all duration-300 hover:border-brand-blue-light/30 hover:translate-y-[-4px] hover:shadow-2xl hover:shadow-black/40 flex flex-col h-full"
    >
      ...
      <div className="p-8 flex flex-col flex-1">
        <div className="text-[10px] font-bold text-brand-blue-light uppercase tracking-[0.2em] mb-3">
          {industry}
        </div>
        <h4 className="text-xl font-bold text-white mb-3 tracking-tight group-hover:text-brand-blue-light transition-colors">
          {title}
        </h4>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          {metrics.map((metric, idx) => (
            <div key={idx} className="flex flex-col">
              <span className="font-display font-extrabold text-2xl text-brand-blue-light tracking-tight">
                {metric.value}
              </span>
