...
export function RetainerCard({ onCtaClick }: { onCtaClick: () => void }) {
  return (
    <div className="bg-gradient-to-br from-navy-mid to-navy-card border border-white/7 rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden group">
      <div className="absolute top-0 right-0 w-64 h-64 bg-brand-blue-light/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 group-hover:bg-brand-blue-light/10 transition-colors" />
      
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 relative z-10 items-center">
        <div>
          <h3 className="text-3xl md:text-5xl font-display font-extrabold text-white mb-6 leading-tight">
            Comprehensive <span className="text-brand-blue-light">Transformation</span> Partnership
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {PERKS.map((perk, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-navy/40 border border-white/5 rounded-2xl p-6 transition-all duration-300 hover:border-brand-blue-light/30 hover:bg-navy/60"
              >
                <div className="w-10 h-10 rounded-xl bg-brand-blue-light/10 flex items-center justify-center text-brand-blue-light mb-4 shadow-[0_4px_12px_rgba(242,138,2,0.1)]">
                  <Icon size={20} />
                </div>
